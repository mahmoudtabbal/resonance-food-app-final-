...
if "dropdown_cache" not in st.session_state:
    st.session_state.dropdown_cache = {}

df = st.session_state.data
i = st.session_state.index
current = df.iloc[i]

# Display item and progress
...
# Use manual dropdown value without triggering jumps
selectbox_key = f"dropdown_{i}"
dropdown_val = st.session_state.dropdown_cache.get(selectbox_key, "")

dropdown_val = st.selectbox(
    "Select Resonance Compatibility",
    ["", "Compatible", "Incompatible", "Limited", "Neutral"],
    index=["", "Compatible", "Incompatible", "Limited", "Neutral"].index(dropdown_val),
    key=f"selectbox_{i}"
)

# Save the selected value manually only when button is clicked
if st.button("Save Compatibility"):
    if dropdown_val:
        st.session_state.dropdown_cache[selectbox_key] = dropdown_val
        st.session_state.data.at[i, "Resonance Compatibility"] = dropdown_val
