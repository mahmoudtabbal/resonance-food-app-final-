...
# Centered layout for logo and chart
try:
...
selected_resonance = st.selectbox(
...
if st.button("Save Compatibility"):
    if st.session_state[res_key]:
        st.session_state.data.at[i, "Resonance Compatibility"] = st.session_state[res_key]
        st.success("Saved! Now click Next Item to continue.")
    else:
...
with col_next:
    ...
    if i < len(df) - 1:
        ...
        st.success("✅ You have reviewed all items!")
