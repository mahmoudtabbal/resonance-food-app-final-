...
# Show and store dropdown value safely
selected_key = f"item_{i}"
default_value = st.session_state.responses.get(selected_key, "")
selected_value = st.selectbox("Select Resonance Compatibility", ["", "Compatible", "Incompatible", "Limited", "Neutral"], index=["", "Compatible", "Incompatible", "Limited", "Neutral"].index(default_value))

# Save button
if st.button("Save Compatibility"):
    if selected_value:
        st.session_state.responses[selected_key] = selected_value
        st.session_state.data.at[i, "Resonance Compatibility"] = selected_value
